...
def install_libraries(libraries):
    for lib in libraries:
        try:
            __import__(lib)  # Intentar importar la librería
        except ImportError:
...
# Contenedor Dry 20' — Relleno progresivo con deslizador (bloque por bloque)
# Requisitos: matplotlib, numpy (sin seaborn)
# Sugerido: %matplotlib widget  (o %matplotlib notebook)
...
# Dimensiones canónicas del bloque (sin rotar): Lb=4*box_l, Wb=5*box_w, Hb=10*box_h
...
# Orientaciones posibles del BLOQUE dentro del contenedor (todas las permutaciones)
# Cada clave muestra (L_bloque, W_bloque, H_bloque)
ORIENTATIONS = {
...
ax.set_title("Contenedor Dry 20'")

# Dibuja el contenedor (envolvente)
add_cuboid(ax, Cuboid(0, 0, 0, CONT_L, CONT_W, CONT_H), alpha=0.03, linewidth=0.6)

# Selector de orientación
rax = plt.axes([0.72, 0.52, 0.26, 0.25])
radio = RadioButtons(rax, tuple(ORIENTATIONS.keys()), active=list(ORIENTATIONS.keys()).index(best_label))

# Deslizador para cantidad de bloques dibujados
sax = plt.axes([0.12, 0.03, 0.6, 0.03])
slider = Slider(sax, "Bloques", valmin=0, valmax=max(best_state["nblocks"], 1), valinit=0, valstep=1)

# Texto resumen (arriba)
summary_ax = plt.axes([0.02, 0.88, 0.96, 0.1])
summary_ax.axis('off')
summary_text = summary_ax.text(0.01, 0.5, "", fontsize=10, va='center', ha='left', family='monospace')

# Estado actual
state = best_state.copy()

def redraw(n_to_draw):
    ax.cla()
    ax.set_title("Contenedor Dry 20' — relleno progresivo con deslizador")
    add_cuboid(ax, Cuboid(0, 0, 0, CONT_L, CONT_W, CONT_H), alpha=0.03, linewidth=0.6)

    # Dibuja los primeros n_to_draw bloques
    n_to_draw = int(max(0, min(n_to_draw, state["nblocks"])))
    for k in range(n_to_draw):
        x0, y0, z0 = state["positions"][k]
        add_cuboid(ax, Cuboid(x0, y0, z0, state["bl_L"], state["bl_W"], state["bl_H"]), alpha=0.22, linewidth=0.5)
    ...
    summary = (
        ...
        f"Dibujados: {n_to_draw} → Cajas: {total_boxes}  |  "
...
def on_radio(label):
    ...
    state.update(st)
    # Actualiza el rango del deslizador a la nueva capacidad
    slider.valmax = max(state["nblocks"], 1)
